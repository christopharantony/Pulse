...
import { detachTask } from '@/features/goals/services/goal-tasks.service';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await detachTask(ctx, new ObjectId(id), new ObjectId(input.taskId));
    return ok(null, 'Task detached');
  } catch (error) {
