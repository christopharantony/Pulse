...
import { restoreTask } from '@/features/tasks/services/tasks.service';
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const task = await restoreTask(ctx, new ObjectId(id));
    return ok(serializeTask(task, null), 'Task restored');
  } catch (error) {
